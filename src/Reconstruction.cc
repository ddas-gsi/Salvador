...
Reconstruction::Reconstruction(char *settings)
{
  ...
  if (fset->VerboseLevel() > 1)
  {
    ...
    {
      cout << "Avg.Interaction.Point for DALI.ID." << i << "\t" << fpositions[i][0] << "\t" << fpositions[i][1] << "\t" << fpositions[i][2] << endl;
    }
    if (fset->DoReCalibration())
...
/*!
  Reads in the DALI time offsets
  \param infile the file with the DALI time offsets
*/
void Reconstruction::ReadDALIToffsets(const char *infile)
{
  std::cout << "Reading DALI Time Offset file: " << infile << std::endl;
  TEnv toff(infile);
  for (int i = 0; i < MAXNCRYSTAL; i++)
  {
...
/*!
  ...
  \param hits a vector with all hits to be recalibrated
*/
void Reconstruction::ReCalDALIToffsets(vector<DALIHit *> hits)
{
  vector<DALIHit *> output;
  for (unsigned short i = 0; i < hits.size(); i++)
  {
